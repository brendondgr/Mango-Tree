"""Thin (de)serialization helpers for the timekeeper API.

The DTOs/validators in ``shared/schemas.py`` already define the schema, so the API
layer only needs request-body guarding and a standard pagination envelope.
"""
...
from utils.apps.timekeeper.shared.constants import DEFAULT_PAGE_SIZE
...
def paginate(request, items: list[dict]) -> dict:
    ...
    dashboard aggregating all logs) may request a larger ``page_size`` up to
    ``MAX_PAGE_SIZE``.
    """
